...
3. Determine the available change-inspection tools.
   - If the repository uses Jujutsu (`.jj` exists) and `jj` is available, follow the `jujutsu` skill for safe VCS commands. For review target selection, remember that the working-copy commit (`@`) may be empty while the actual change to review is its parent (`@-`). If the apparent target is empty, inspect the nearby history before assuming there is nothing to review; if the parent appears to be trunk/main or there are multiple plausible non-empty ancestors, ask the user for the target instead of assuming.
